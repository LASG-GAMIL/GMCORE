...
contains
  ...
  subroutine mountain_zonal_flow_test_set_initial_condition()
    ...
    call parallel_fill_halo(mesh, state(1)%u, all_halo=.true.)
    ...
    call parallel_fill_halo(mesh, state(1)%v, all_halo=.true.)
    ...
    call parallel_fill_halo(mesh, state(1)%gd, all_halo=.true.)
