...
module simple_physics_mod

  use const_mod, only: r8     => r8     , &
                       pi     => pi     , &
                       omega  => omega  , &
                       gravit => g      , &
                       a      => radius , &
...
contains
  ...
  subroutine simple_physics_run(pcols, pver, dtime, lat, t, q, u, v, pmid, pint, &
                                pdel, rpdel, ps, precl, test, RJ2012_precip, TC_PBL_mod)
    ...
    !
    if (test == 1) then ! Moist Baroclinic Wave Test
      do i = 1, pcols
    ...
    else if (test == 0) then ! Tropical Cyclone Test
      do i = 1, pcols
        Tsurf(i) = SST_TC
      end do
    end if
