...
contains
  ...
  subroutine process_init()
    ...
    integer ierr, i, j
    integer nproc, proc_dims(2), proc_coords(2)
    ...
    proc_dims = 0
    call MPI_DIMS_CREATE(nproc, 2, proc_dims, ierr)
    call MPI_CART_CREATE(MPI_COMM_WORLD, 2, proc_dims, [.true.,.false.], .true., proc%comm, ierr)
    ...
    res_num = mod(global_mesh%num_full_lon, proc_dims(1))
    lon_ibeg = 1
    do i = 0, proc_coords(1) - 1
      if (res_num /= 0 .and. i < res_num - 1) then
        num_lon = global_mesh%num_full_lon / proc_dims(1) + 1
      else
        num_lon = global_mesh%num_full_lon / proc_dims(1)
      end if
...
#ifdef V_POLE
#else
    res_num = mod(global_mesh%num_full_lat, proc_dims(2))
    half_num = proc_dims(2) - proc_dims(2) / 2
    lat_ibeg = 1
    do j = 0, proc_coords(2) - 1
      if (res_num /= 0 .and. j >= half_num .and. j < half_num + res_num) then
        num_lat = global_mesh%num_full_lat / proc_dims(2) + 1
      else
        num_lat = global_mesh%num_full_lat / proc_dims(2)
      end if
