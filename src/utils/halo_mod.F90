...
contains
  ...
  subroutine halo_init_normal(this, mesh, ngb_proc_id, iblk, west_lat_ibeg, west_lat_iend, &
    ...
    array_size(:,1,1) = [mesh%num_full_lon+2*mesh%lon_halo_width,mesh%num_full_lat+2*mesh%lat_halo_width]
    array_size(:,2,1) = [mesh%num_half_lon+2*mesh%lon_halo_width,mesh%num_full_lat+2*mesh%lat_halo_width]
    array_size(:,1,2) = [mesh%num_full_lon+2*mesh%lon_halo_width,mesh%num_half_lat+2*mesh%lat_halo_width]
    array_size(:,2,2) = [mesh%num_half_lon+2*mesh%lon_halo_width,mesh%num_half_lat+2*mesh%lat_halo_width]
    if (present(west_lat_ibeg) .and. present(west_lat_iend)) then
      this%west_lat_ibeg = west_lat_ibeg
      this%west_lat_iend = west_lat_iend
      ! full_lon + full_lat
      send_subarray_size (:,1,1) = [mesh%lon_halo_width,mesh%num_full_lat]
      recv_subarray_size (:,1,1) = send_subarray_size(:,1,1)
      send_subarray_start(:,1,1) = [mesh%lon_halo_width,mesh%lat_halo_width]
      recv_subarray_start(:,1,1) = [                  0,mesh%lat_halo_width]
      ! half_lon + full_lat
      send_subarray_size (:,2,1) = [mesh%lon_halo_width,mesh%num_full_lat]
      recv_subarray_size (:,2,1) = send_subarray_size(:,2,1)
      send_subarray_start(:,2,1) = [mesh%lon_halo_width,mesh%lat_halo_width]
      recv_subarray_start(:,2,1) = [                  0,mesh%lat_halo_width]
      ! full_lon + half_lat
      send_subarray_size (:,1,2) = [mesh%lon_halo_width,mesh%num_half_lat]
      recv_subarray_size (:,1,2) = send_subarray_size(:,1,2)
      send_subarray_start(:,1,2) = [mesh%lon_halo_width,mesh%lat_halo_width]
      recv_subarray_start(:,1,2) = [                  0,mesh%lat_halo_width]
      ! half_lon + half_lat
      send_subarray_size (:,2,2) = [mesh%lon_halo_width,mesh%num_half_lat]
      recv_subarray_size (:,2,2) = send_subarray_size(:,2,2)
      send_subarray_start(:,2,2) = [mesh%lon_halo_width,mesh%lat_halo_width]
      recv_subarray_start(:,2,2) = [                  0,mesh%lat_halo_width]
      ...
      send_subarray_size (:,1,1) = [mesh%lon_halo_width,mesh%num_full_lat]
      recv_subarray_size (:,1,1) = send_subarray_size(:,1,1)
      send_subarray_start(:,1,1) = [mesh%num_full_lon                    ,mesh%lat_halo_width]
      recv_subarray_start(:,1,1) = [mesh%num_full_lon+mesh%lon_halo_width,mesh%lat_halo_width]
      ! half_lon + full_lat
      send_subarray_size (:,2,1) = [mesh%lon_halo_width,mesh%num_full_lat]
      recv_subarray_size (:,2,1) = send_subarray_size(:,2,1)
      send_subarray_start(:,2,1) = [mesh%num_half_lon                    ,mesh%lat_halo_width]
      recv_subarray_start(:,2,1) = [mesh%num_half_lon+mesh%lon_halo_width,mesh%lat_halo_width]
      ! full_lon + half_lat
      send_subarray_size (:,1,2) = [mesh%lon_halo_width,mesh%num_half_lat]
      recv_subarray_size (:,1,2) = send_subarray_size(:,1,2)
      send_subarray_start(:,1,2) = [mesh%num_full_lon                    ,mesh%lat_halo_width]
      recv_subarray_start(:,1,2) = [mesh%num_full_lon+mesh%lon_halo_width,mesh%lat_halo_width]
      ! half_lon + half_lat
      send_subarray_size (:,2,2) = [                  mesh%lon_halo_width,mesh%num_half_lat  ]
      recv_subarray_size (:,2,2) = send_subarray_size(:,2,2)
      send_subarray_start(:,2,2) = [mesh%num_half_lon                    ,mesh%lat_halo_width]
      recv_subarray_start(:,2,2) = [mesh%num_half_lon+mesh%lon_halo_width,mesh%lat_halo_width]
      ...
      ! full_lon + full_lat
      send_subarray_size (:,1,1) = [array_size(1,1,1),mesh%lat_halo_width]
      recv_subarray_size (:,1,1) = send_subarray_size(:,1,1)
      send_subarray_start(:,1,1) = [0,mesh%lat_halo_width]
      recv_subarray_start(:,1,1) = [0,0]
      ! half_lon + full_lat
      send_subarray_size (:,2,1) = [array_size(1,2,1),mesh%lat_halo_width]
      recv_subarray_size (:,2,1) = send_subarray_size(:,2,1)
      send_subarray_start(:,2,1) = [0,mesh%lat_halo_width]
      recv_subarray_start(:,2,1) = [0,0]
      ! full_lon + half_lat
      send_subarray_size (:,1,2) = [array_size(1,1,2),mesh%lat_halo_width]
      recv_subarray_size (:,1,2) = send_subarray_size(:,1,2)
      send_subarray_start(:,1,2) = [0,mesh%lat_halo_width]
      recv_subarray_start(:,1,2) = [0,0]
      ! half_lon + half_lat
      send_subarray_size (:,2,2) = [array_size(1,2,2),mesh%lat_halo_width]
      recv_subarray_size (:,2,2) = send_subarray_size(:,2,2)
      send_subarray_start(:,2,2) = [0,mesh%lat_halo_width]
      recv_subarray_start(:,2,2) = [0,0]
      ...
      send_subarray_size (:,1,1) = [array_size(1,1,1),mesh%lat_halo_width]
      recv_subarray_size (:,1,1) = send_subarray_size(:,1,1)
      send_subarray_start(:,1,1) = [0,mesh%num_full_lat]
      recv_subarray_start(:,1,1) = [0,mesh%num_full_lat+mesh%lat_halo_width]
      ! half_lon + full_lat
      send_subarray_size (:,2,1) = [array_size(1,2,1),mesh%lat_halo_width]
      recv_subarray_size (:,2,1) = send_subarray_size(:,2,1)
      send_subarray_start(:,2,1) = [0,mesh%num_full_lat]
      recv_subarray_start(:,2,1) = [0,mesh%num_full_lat+mesh%lat_halo_width]
      ! full_lon + half_lat
      send_subarray_size (:,1,2) = [array_size(1,1,2),mesh%lat_halo_width]
      recv_subarray_size (:,1,2) = send_subarray_size(:,1,2)
      send_subarray_start(:,1,2) = [0,mesh%num_half_lat]
      recv_subarray_start(:,1,2) = [0,mesh%num_half_lat+mesh%lat_halo_width]
      ! half_lon + half_lat
      send_subarray_size (:,2,2) = [array_size(1,2,2),mesh%lat_halo_width]
      recv_subarray_size (:,2,2) = send_subarray_size(:,2,2)
      send_subarray_start(:,2,2) = [0,mesh%num_half_lat]
      recv_subarray_start(:,2,2) = [0,mesh%num_half_lat+mesh%lat_halo_width]
    end if
