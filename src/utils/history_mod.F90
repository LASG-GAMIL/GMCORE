...
contains
  ...
  subroutine history_init()
    ...
    call io_add_var('h1', 'dpedlon'  , long_name='zonal geopotential energy gradient force'     , units=''       , dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('h1', 'dkedlon'  , long_name='zonal kinetic energy gradient force'          , units=''       , dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('h1', 'dpedlat'  , long_name='meridional geopotential energy gradient force', units=''       , dim_names=['lon ', 'ilat', 'time'])
    call io_add_var('h1', 'dkedlat'  , long_name='meridional kinetic energy gradient force'     , units=''       , dim_names=['lon ', 'ilat', 'time'])
    ...
    call io_add_var('h1', 'mf_lon_n' , long_name='normal mass flux on U grid'                   , units='m2 s-1' , dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('h1', 'mf_lon_t' , long_name='tangent mass flux on U grid'                  , units='m2 s-1' , dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('h1', 'mf_lat_n' , long_name='normal mass flux on V grid'                   , units='m2 s-1' , dim_names=['lon ', 'ilat', 'time'])
    call io_add_var('h1', 'mf_lat_t' , long_name='tangent mass flux on V grid'                  , units='m2 s-1' , dim_names=['lon ', 'ilat', 'time'])
    ...
    call io_add_var('h1', 'dpv_lon_t', long_name='meridional dpv on U grid'                     , units='m-1 s-1', dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('h1', 'dpv_lon_n', long_name='zonal dpv on U grid'                          , units='m-1 s-1', dim_names=['ilon', 'lat ', 'time'])
    call io_add_var('h1', 'dpv_lat_t', long_name='zonal dpv on V grid'                          , units='m-1 s-1', dim_names=['lon ', 'ilat', 'time'])
    call io_add_var('h1', 'dpv_lat_n', long_name='meridional dpv on V grid'                     , units='m-1 s-1', dim_names=['lon ', 'ilat', 'time'])
    call io_add_var('h1', 'ke'       , long_name='kinetic energy on cell grid'                  , units='',        dim_names=['lon ', 'lat ', 'time'])
  ...
  subroutine history_write_debug(static, state, tend)
    ...
    call io_output('h1', 'dpedlon'  , tend%dpedlon     (1:mesh%num_half_lon,1:mesh%num_full_lat))
    call io_output('h1', 'dkedlon'  , tend%dkedlon     (1:mesh%num_half_lon,1:mesh%num_full_lat))
    call io_output('h1', 'dpedlat'  , tend%dpedlat     (1:mesh%num_half_lon,1:mesh%num_full_lat))
    call io_output('h1', 'dkedlat'  , tend%dkedlat     (1:mesh%num_half_lon,1:mesh%num_full_lat))
    ...
    call io_output('h1', 'mf_lon_t' , state%mf_lon_t   (1:mesh%num_half_lon,1:mesh%num_full_lat))
    call io_output('h1', 'mf_lat_n' , state%mf_lat_n   (1:mesh%num_full_lon,1:mesh%num_half_lat))
    call io_output('h1', 'mf_lat_t' , state%mf_lat_t   (1:mesh%num_full_lon,1:mesh%num_half_lat))
    ...
    call io_output('h1', 'dpv_lon_t', state%dpv_lon_t  (1:mesh%num_full_lon,1:mesh%num_half_lat))
    call io_output('h1', 'dpv_lon_n', state%dpv_lon_n  (1:mesh%num_full_lon,1:mesh%num_half_lat))
    call io_output('h1', 'dpv_lat_t', state%dpv_lat_t  (1:mesh%num_half_lon,1:mesh%num_full_lat))
    call io_output('h1', 'dpv_lat_n', state%dpv_lat_n  (1:mesh%num_half_lon,1:mesh%num_full_lat))
    call io_output('h1', 'ke'       , state%ke         (1:mesh%num_full_lon,1:mesh%num_full_lat))
