...
contains
  ...
  subroutine filter_init(this, mesh)
    ...
    allocate(this%wgt_lon(maxval(this%ngrid_lon),mesh%full_lat_lb:mesh%full_lat_ub))
    this%wgt_lon = 0
    ...
    allocate(this%wgt_lat(maxval(this%ngrid_lat),mesh%half_lat_lb:mesh%half_lat_ub))
    this%wgt_lat = 0
    do j = mesh%half_lat_lb, mesh%half_lat_ub
