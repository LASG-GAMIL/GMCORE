...
#else
      if (j == mesh%half_lat_start_idx .and. reduced_full_mesh(j+1)%reduce_factor > 0) then
        state%v(:,j) = 0.1 * state%v(:,j+1) + 0.9 * state%v(:,j)
      else if (j == mesh%half_lat_end_idx .and. reduced_full_mesh(j  )%reduce_factor > 0) then
        state%v(:,j) = 0.1 * state%v(:,j-1) + 0.9 * state%v(:,j)
