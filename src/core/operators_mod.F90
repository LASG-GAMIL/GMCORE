...
contains
  ...
  subroutine calc_voru_vorv(block, state, tend, dt)
    ...
    do k = mesh%full_lev_ibeg, mesh%full_lev_iend
      do j = mesh%half_lat_ibeg_no_pole, mesh%half_lat_iend_no_pole
        if (block%reduced_mesh(j-1)%reduce_factor > 0) then
          reduced_mesh  => block%reduced_mesh (j-1)
          reduced_tend  => block%reduced_tend (j-1)
          reduced_state => block%reduced_state(j-1)
          tend%voru(:,j,k) = 0.0_r8
          do move = 1, reduced_mesh%reduce_factor
            call wait_halo(reduced_state%async(async_pv_lon,0,move))
            do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
              reduced_tend%voru(i,k) = (                                                                              &
                reduced_mesh%half_tangent_wgt(1,1) * (                                                                &
                  reduced_state%mf_lon_n(k,i-1,0,move) * (                                                            &
                    reduced_state%pv_lat(k,i  ,1,move) - reduced_mesh%half_f(1) / reduced_state%m_lat(k,i  ,1,move) + &
                    reduced_state%pv_lon(k,i-1,0,move) - reduced_mesh%full_f(0) / reduced_state%m_lon(k,i-1,0,move)   &
                  ) +                                                                                                 &
                  reduced_state%mf_lon_n(k,i  ,0,move) * (                                                            &
                    reduced_state%pv_lat(k,i  ,1,move) - reduced_mesh%half_f(1) / reduced_state%m_lat(k,i  ,1,move) + &
                    reduced_state%pv_lon(k,i  ,0,move) - reduced_mesh%full_f(0) / reduced_state%m_lon(k,i  ,0,move)   &
                  )                                                                                                   &
                )                                                                                                     &
              ) * 0.5_r8
            end do
            call reduce_append_array(move, reduced_mesh, reduced_tend%voru(:,k), mesh, tend%voru(:,j,k))
          end do
          call overlay_inner_halo(block, tend%voru(:,j,k), west_halo=.true.)
        else
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            if (coriolis_scheme == 1) then
              tend%voru(i,j,k) = (                                                        &
                mesh%half_tangent_wgt(1,j) * (                                            &
                  state%mf_lon_n(i-1,j-1,k) * (                                           &
                    state%pv_lat(i  ,j  ,k) - mesh%half_f(j  ) / state%m_lat(i  ,j  ,k) + &
                    state%pv_lon(i-1,j-1,k) - mesh%full_f(j-1) / state%m_lon(i-1,j  ,k)   &
                  ) +                                                                     &
                  state%mf_lon_n(i  ,j-1,k) * (                                           &
                    state%pv_lat(i  ,j  ,k) - mesh%half_f(j  ) / state%m_lat(i  ,j  ,k) + &
                    state%pv_lon(i  ,j-1,k) - mesh%half_f(j-1) / state%m_lon(i  ,j-1,k)   &
                  )                                                                       &
                )                                                                         &
              ) * 0.5_r8
            else if (coriolis_scheme == 2) then
              tend%voru(i,j,k) = state%mf_lat_t(i,j,k) * (state%pv_lat(i,j,k) - mesh%half_f(j) / state%m_lat(i,j,k))
            end if
          end do
        end if
        if (block%reduced_mesh(j)%reduce_factor > 0) then
          ...
          call zero_halo(block, tend%voru(:,j,k), east_halo=.true.)
          do move = 1, reduced_mesh%reduce_factor
            call wait_halo(reduced_state%async(async_pv_lon,0,move))
            do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
              reduced_tend%voru(i,k) = (                                                                              &
                reduced_mesh%half_tangent_wgt(2,0) * (                                                                &
                  reduced_state%mf_lon_n(k,i-1,0,move) * (                                                            &
                    reduced_state%pv_lat(k,i  ,0,move) - reduced_mesh%half_f(0) / reduced_state%m_lat(k,i  ,0,move) + &
                    reduced_state%pv_lon(k,i-1,0,move) - reduced_mesh%full_f(0) / reduced_state%m_lon(k,i-1,0,move)   &
                  ) +                                                                                                 &
                  reduced_state%mf_lon_n(k,i  ,0,move) * (                                                            &
                    reduced_state%pv_lat(k,i  ,0,move) - reduced_mesh%half_f(0) / reduced_state%m_lat(k,i  ,0,move) + &
          ...
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            if (coriolis_scheme == 1) then
              tend%voru(i,j,k) = tend%voru(i,j,k) + (                                   &
                mesh%half_tangent_wgt(2,j) * (                                          &
                  state%mf_lon_n(i-1,j  ,k) * (                                         &
                    state%pv_lat(i  ,j  ,k) - mesh%half_f(j) / state%m_lat(i  ,j  ,k) + &
                    state%pv_lon(i-1,j  ,k) - mesh%full_f(j) / state%m_lon(i-1,j  ,k)   &
                  ) +                                                                   &
                  state%mf_lon_n(i  ,j  ,k) * (                                         &
                    state%pv_lat(i  ,j  ,k) - mesh%half_f(j) / state%m_lat(i  ,j  ,k) + &
                    state%pv_lon(i  ,j  ,k) - mesh%full_f(j) / state%m_lon(i  ,j  ,k)   &
                  )                                                                     &
                )                                                                       &
              ) * 0.5_r8
            else if (coriolis_scheme == 2) then
      ...
      do j = mesh%half_lat_ibeg, mesh%half_lat_iend
        if (block%reduced_mesh(j)%reduce_factor > 0) then
          ...
          do move = 1, reduced_mesh%reduce_factor
            ...
            do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
              reduced_tend%voru(i,k) = (                                                                      &
                reduced_mesh%half_tangent_wgt(1,0) * (                                                        &
                  reduced_state%mf_lon_n(k,i-1,0,move) * (                                                    &
                    reduced_state%pv_lat(k,i  ,0,move) - mesh%half_f(j) / reduced_state%m_lat(k,i  ,0,move) + &
                    reduced_state%pv_lon(k,i-1,0,move) - mesh%full_f(j) / reduced_state%m_lon(k,i-1,0,move)   &
                  ) +                                                                                         &
                  reduced_state%mf_lon_n(k,i  ,0,move) * (                                                    &
                    reduced_state%pv_lat(k,i  ,0,move) - mesh%half_f(j) / reduced_state%m_lat(k,i  ,0,move) + &
                    reduced_state%pv_lon(k,i  ,0,move) - mesh%full_f(j) / reduced_state%m_lon(k,i  ,0,move)   &
                  )                                                                                           &
                )                                                                                             &
              ) * 0.5_r8
            end do
          ...
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            if (j > global_mesh%half_lat_ibeg) then
              tend%voru(i,j,k) = (                                                      &
                mesh%half_tangent_wgt(1,j) * (                                          &
                  state%mf_lon_n(i-1,j  ,k) * (                                         &
                    state%pv_lat(i  ,j  ,k) - mesh%half_f(j) / state%m_lat(i  ,j  ,k) + &
        ...
        if (block%reduced_mesh(j+1)%reduce_factor > 0) then
          ...
          do move = 1, reduced_mesh%reduce_factor
            ...
            do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
              reduced_tend%voru(i,k) = (                                                                          &
                reduced_mesh%half_tangent_wgt(2,-1) * (                                                           &
                  reduced_state%mf_lon_n(k,i-1, 0,move) * (                                                       &
                    reduced_state%pv_lat(k,i  ,-1,move) - mesh%half_f(j  ) / reduced_state%m_lat(k,i  ,-1,move) + &
                    reduced_state%pv_lon(k,i-1, 0,move) - mesh%full_f(j+1) / reduced_state%m_lon(k,i-1, 0,move)   &
                  ) +                                                                                             &
                  reduced_state%mf_lon_n(k,i  , 0,move) * (                                                       &
                    reduced_state%pv_lat(k,i  ,-1,move) - mesh%half_f(j  ) / reduced_state%m_lat(k,i  ,-1,move) + &
                    reduced_state%pv_lon(k,i  , 0,move) - mesh%full_f(j+1) / reduced_state%m_lon(k,i  , 0,move)   &
                  )                                                                                               &
                )                                                                                                 &
              ) * 0.5_r8
            end do
          ...
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            if (j < global_mesh%half_lat_iend) then
              tend%voru(i,j,k) = tend%voru(i,j,k) + (                                     &
                mesh%half_tangent_wgt(2,j) * (                                            &
                  state%mf_lon_n(i-1,j+1,k) * (                                           &
                    state%pv_lat(i  ,j  ,k) - mesh%half_f(j  ) / state%m_lat(i  ,j  ,k) + &
                    state%pv_lon(i-1,j+1,k) - mesh%full_f(j+1) / state%m_lon(i-1,j+1,k)   &
                  ) +                                                                     &
                  state%mf_lon_n(i  ,j+1,k) * (                                           &
                    state%pv_lat(i  ,j  ,k) - mesh%half_f(j  ) / state%m_lat(i  ,j  ,k) + &
                    state%pv_lon(i  ,j+1,k) - mesh%full_f(j+1) / state%m_lon(i  ,j+1,k)   &
                  )                                                                       &
                )                                                                         &
              ) * 0.5_r8
            end if
    ...
    do k = mesh%full_lev_ibeg, mesh%full_lev_iend
      do j = mesh%full_lat_ibeg, mesh%full_lat_iend
        if (block%reduced_mesh(j)%reduce_factor > 0) then
          ...
          reduced_tend  => block%reduced_tend (j)
          reduced_state => block%reduced_state(j)
          tend%vorv(:,j,k) = 0.0_r8
          do move = 1, reduced_mesh%reduce_factor
            call wait_halo(reduced_state%async(async_pv_lat,0,move))
            call wait_halo(reduced_state%async(async_pv_lat,1,move))
            do i = reduced_mesh%half_lon_ibeg, reduced_mesh%half_lon_iend
              reduced_tend%vorv(i,k) = (                                                                        &
                reduced_mesh%full_tangent_wgt(1,0) * (                                                          &
                  reduced_state%mf_lat_n(k,i  ,0,move) * (                                                      &
                    reduced_state%pv_lon(k,i  ,0,move) - mesh%full_f(j  ) / reduced_state%m_lon(k,i  ,0,move) + &
                    reduced_state%pv_lat(k,i  ,0,move) - mesh%half_f(j  ) / reduced_state%m_lat(k,i  ,0,move)   &
                  ) +                                                                                           &
                  reduced_state%mf_lat_n(k,i+1,0,move) * (                                                      &
                    reduced_state%pv_lon(k,i  ,0,move) - mesh%full_f(j  ) / reduced_state%m_lon(k,i  ,0,move) + &
                    reduced_state%pv_lat(k,i+1,0,move) - mesh%half_f(j  ) / reduced_state%m_lat(k,i+1,0,move)   &
                  )                                                                                             &
                ) +                                                                                             &
                reduced_mesh%full_tangent_wgt(2,0) * (                                                          &
                  reduced_state%mf_lat_n(k,i  ,1,move) * (                                                      &
                    reduced_state%pv_lon(k,i  ,0,move) - mesh%full_f(j  ) / reduced_state%m_lon(k,i  ,0,move) + &
                    reduced_state%pv_lat(k,i  ,1,move) - mesh%half_f(j+1) / reduced_state%m_lat(k,i  ,1,move)   &
                  ) +                                                                                           &
                  reduced_state%mf_lat_n(k,i+1,1,move) * (                                                      &
                    reduced_state%pv_lon(k,i  ,0,move) - mesh%full_f(j  ) / reduced_state%m_lon(k,i  ,0,move) + &
                    reduced_state%pv_lat(k,i+1,1,move) - mesh%half_f(j+1) / reduced_state%m_lat(k,i+1,1,move)   &
                  )                                                                                             &
                )                                                                                               &
              ) * 0.5_r8
            end do
            call reduce_append_array(move, reduced_mesh, reduced_tend%vorv(:,k), mesh, tend%vorv(:,j,k))
          end do
          call overlay_inner_halo(block, tend%vorv(:,j,k), west_halo=.true.)
        else
          do i = mesh%half_lon_ibeg, mesh%half_lon_iend
            if (coriolis_scheme == 1) then
              tend%vorv(i,j,k) = (                                                        &
                mesh%full_tangent_wgt(1,j) * (                                            &
                  state%mf_lat_n(i  ,j  ,k) * (                                           &
                    state%pv_lon(i  ,j  ,k) - mesh%full_f(j  ) / state%m_lon(i  ,j  ,k) + &
                    state%pv_lat(i  ,j  ,k) - mesh%half_f(j  ) / state%m_lat(i  ,j  ,k)   &
                  ) +                                                                     &
                  state%mf_lat_n(i+1,j  ,k) * (                                           &
                    state%pv_lon(i  ,j  ,k) - mesh%full_f(j  ) / state%m_lon(i  ,j  ,k) + &
                    state%pv_lat(i+1,j  ,k) - mesh%half_f(j  ) / state%m_lat(i+1,j  ,k)   &
                  )                                                                       &
                ) +                                                                       &
                mesh%full_tangent_wgt(2,j) * (                                            &
                  state%mf_lat_n(i  ,j+1,k) * (                                           &
                    state%pv_lon(i  ,j  ,k) - mesh%full_f(j  ) / state%m_lon(i  ,j  ,k) + &
                    state%pv_lat(i  ,j+1,k) - mesh%half_f(j+1) / state%m_lat(i  ,j+1,k)   &
                  ) +                                                                     &
                  state%mf_lat_n(i+1,j+1,k) * (                                           &
                    state%pv_lon(i  ,j  ,k) - mesh%full_f(j  ) / state%m_lon(i  ,j  ,k) + &
                    state%pv_lat(i+1,j+1,k) - mesh%half_f(j+1) / state%m_lat(i+1,j+1,k)   &
                  )                                                                       &
                )                                                                         &
              ) * 0.5_r8
            else if (coriolis_scheme == 2) then
              tend%vorv(i,j,k) = state%mf_lon_t(i,j,k) * (state%pv_lon(i,j,k) - mesh%full_f(j) / state%m_lon(i,j,k))
            end if
          end do
        end if
      end do
  ...
  end subroutine calc_voru_vorv

  subroutine calc_fu_fv(block, state, tend, dt)
    ...
    do k = mesh%full_lev_ibeg, mesh%full_lev_iend
      do j = mesh%half_lat_ibeg_no_pole, mesh%half_lat_iend_no_pole
        if (block%reduced_mesh(j-1)%reduce_factor > 0) then
          ...
          do move = 1, reduced_mesh%reduce_factor
            do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
              reduced_tend%fu(i,k) = reduced_mesh%half_tangent_wgt(1,1) * ( &
                reduced_state%u(k,i-1,0,move) + reduced_state%u(k,i,0,move) &
              ) * mesh%full_f(j-1)
            end do
            call reduce_append_array(move, reduced_mesh, reduced_tend%fu(:,k), mesh, tend%fu(:,j,k))
          end do
          call overlay_inner_halo(block, tend%fu(:,j,k), west_halo=.true.)
        else
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            if (coriolis_scheme == 1) then
              tend%fu(i,j,k) = mesh%half_tangent_wgt(1,j) * (state%u(i-1,j-1,k) + state%u(i,j-1,k)) * mesh%full_f(j-1)
            else if (coriolis_scheme == 2) then
              tend%fu(i,j,k) = state%mf_lat_t(i,j,k) / state%m_lat(i,j,k) * mesh%full_f(j-1)
            end if
          end do
        end if
        if (block%reduced_mesh(j)%reduce_factor > 0) then
          ...
          reduced_tend  => block%reduced_tend (j)
          reduced_state => block%reduced_state(j)
          call zero_halo(block, tend%fu(:,j,k), east_halo=.true.)
          do move = 1, reduced_mesh%reduce_factor
            do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
              reduced_tend%fu(i,k) = reduced_mesh%half_tangent_wgt(2,0) * ( &
                reduced_state%u(k,i-1,0,move) + reduced_state%u(k,i,0,move) &
              ) * mesh%full_f(j)
            end do
            call reduce_append_array(move, reduced_mesh, reduced_tend%fu(:,k), mesh, tend%fu(:,j,k))
          ...
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            if (coriolis_scheme == 1) then
              tend%fu(i,j,k) = tend%fu(i,j,k) + mesh%half_tangent_wgt(2,j) * (state%u(i-1,j,k) + state%u(i,j,k)) * mesh%full_f(j)
            else if (coriolis_scheme == 2) then
              tend%fu(i,j,k) = state%mf_lat_t(i,j,k) / state%m_lat(i,j,k) * mesh%full_f(j)
            end if
          end do
        end if
      end do
    ...
    do k = mesh%full_lev_ibeg, mesh%full_lev_iend
      do j = mesh%half_lat_ibeg, mesh%half_lat_iend
        if (block%reduced_mesh(j)%reduce_factor > 0) then
          ...
          do move = 1, reduced_mesh%reduce_factor
            do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
              reduced_tend%fu(i,k) = reduced_mesh%half_tangent_wgt(1,0) * ( &
                reduced_state%u(k,i-1,0,move) + reduced_state%u(k,i,0,move) &
              ) * mesh%full_f(j)
            end do
            call reduce_append_array(move, reduced_mesh, reduced_tend%fu(:,k), mesh, tend%fu(:,j,k))
          ...
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            if (coriolis_scheme == 1) then
              tend%fu(i,j,k) = mesh%half_tangent_wgt(1,j) * (state%u(i-1,j,k) + state%u(i,j,k)) * mesh%full_f(j)
            else if (coriolis_scheme == 2) then
              tend%fu(i,j,k) = state%mf_lat_t(i,j,k) / state%m_lat(i,j,k) * mesh%full_f(j)
            end if
          end do
        ...
        if (block%reduced_mesh(j+1)%reduce_factor > 0) then
          ...
          do move = 1, reduced_mesh%reduce_factor
            do i = reduced_mesh%full_lon_ibeg, reduced_mesh%full_lon_iend
              reduced_tend%fu(i,k) = reduced_mesh%half_tangent_wgt(2,-1) * ( &
                reduced_state%u(k,i-1,0,move) + reduced_state%u(k,i,0,move)  &
              ) * mesh%full_f(j+1)
            end do
            call reduce_append_array(move, reduced_mesh, reduced_tend%fu(:,k), mesh, tend%fu(:,j,k))
          ...
          do i = mesh%full_lon_ibeg, mesh%full_lon_iend
            if (coriolis_scheme == 1) then
              tend%fu(i,j,k) = tend%fu(i,j,k) + mesh%half_tangent_wgt(2,j) * (state%u(i-1,j+1,k) + state%u(i,j+1,k)) * mesh%full_f(j+1)
            else if (coriolis_scheme == 2) then
              tend%fu(i,j,k) = state%mf_lat_t(i,j,k) / state%m_lat(i,j,k) * mesh%full_f(j+1)
            end if
          end do
        ...
        if (block%reduced_mesh(j)%reduce_factor > 0) then
          ...
          do move = 1, reduced_mesh%reduce_factor
            do i = reduced_mesh%half_lon_ibeg, reduced_mesh%half_lon_iend
              reduced_tend%fv(i,k) =                                          &
                reduced_mesh%full_tangent_wgt(1,0) * (                        &
                  reduced_state%v(k,i,0,move) + reduced_state%v(k,i+1,0,move) &
                ) * mesh%half_f(j) +                                          &
                reduced_mesh%full_tangent_wgt(2,0) * (                        &
                  reduced_state%v(k,i,1,move) + reduced_state%v(k,i+1,1,move) &
                ) * mesh%half_f(j+1)
            end do
            call reduce_append_array(move, reduced_mesh, reduced_tend%fv(:,k), mesh, tend%fv(:,j,k))
          end do
          call overlay_inner_halo(block, tend%fv(:,j,k), west_halo=.true.)
        else
          do i = mesh%half_lon_ibeg, mesh%half_lon_iend
            if (coriolis_scheme == 1) then
              tend%fv(i,j,k) = mesh%full_tangent_wgt(1,j) * (     &
                state%v(i,j  ,k) + state%v(i+1,j  ,k)             &
              ) * mesh%half_f(j) + mesh%full_tangent_wgt(2,j) * ( &
                state%v(i,j+1,k) + state%v(i+1,j+1,k)             &
              ) * mesh%half_f(j+1)
            else if (coriolis_scheme == 2) then
              tend%fv(i,j,k) = state%mf_lon_t(i,j,k) / state%m_lon(i,j,k) * mesh%full_f(j)
            end if
          end do
        end if
      end do
        ...
        if (block%reduced_mesh(j)%reduce_factor > 0) then
          ...
          do i = mesh%half_lon_ibeg, mesh%half_lon_iend
            if (coriolis_scheme == 1) then
              tend%fv(i,j,k) = mesh%full_tangent_wgt(1,j) * (       &
                state%v(i,j-1,k) + state%v(i+1,j-1,k)               &
              ) * mesh%half_f(j-1) + mesh%full_tangent_wgt(2,j) * ( &
                state%v(i,j  ,k) + state%v(i+1,j  ,k)               &
              ) * mesh%half_f(j)
            else if (coriolis_scheme == 2) then
              tend%fv(i,j,k) = state%mf_lon_t(i,j,k) / state%m_lon(i,j,k) * mesh%full_f(j)
